export const en = {
  'web-app': {
    'cart': {
      'title': 'Cart',
      'schedule': 'Schedule',
      'delivery': 'Delivery',
      'delivery-details': 'Delivery details',
      'delivery-available-until': 'You can order delivery until',
      'pickup': 'Pickup',
      'pickup-details': 'Pickup details',
      'pickup-available-until': 'You can order pickup until',
      'next-label': 'Okay, next',
      'conditions': 'Detailed conditions',
      'discount': 'Discount',
      'empty-label': 'Your cart is empty',
      'from': 'from',
      'free-from': 'free from',
      'today-until': 'today until',
      'prepare-for': 'Prepare for',
      'deliver-by': 'Deliver by',
      'now': 'Now',
      'left': 'Left',
      'add': 'Add',
      'add-to-cart': 'Add to cart',
    },
    'checkout': {
      'title': 'Placing an order',
      'contacts': 'Contacts',
      'your-phone': 'Your phone',
      'your-name': 'Your name',
      'delivery-address': 'Delivery address',
      'pickup-address': 'Pickup address',
      'address': {
        'title': 'Address',
        'street': 'Street and house',
        'flat': 'Apt./office',
        'intercom': 'Intercom',
        'entrance': 'Entrance',
        'floor': 'Floor',
        'note': 'Note',
        'note-placeholder': 'For faster delivery, specify additional information',
      },
      'time-title': 'Time',
      'select-time-title': 'Choose a time',
      'selected-time-unavailable': 'Selected time is no longer available',
      'selected-time-unavailable-description': 'Please choose another time',
      'on-time': 'On time',
      'as-soon-as-possible': 'As soon as possible',
      'order-title': 'You order',
      'order-note': 'Note for kitchen',
      'order-note-placeholder': 'Wishes for order',
      'payment-title': 'Payment method',
      'total-title': 'Total',
      'create-order': 'Create order',
      'cost': {
        delivery: 'Delivery cost',
        products: 'Cost of goods',
      },
      'change-label': 'From what amount should prepare change',
      'delivery-cost': 'Delivery cost',
      'delivery-zone-not-found': 'Delivery to this address is not available',
      'info-shipping-price': 'The website does not take shipping costs into account. Totals may vary.',
      'warning-data': 'Please fill in all required fields',
    },
    'finish': {
      'title': 'Order placed',
      'success-message': 'Your request has been successfully submitted for processing.',
      'expect-call': 'Expect a call from the operator.',
      'ordered-title': 'You ordered',
    },
    'search': {
      'placeholder': 'Find a dish',
      'nothing-found': 'Nothing found',
      'most-often': 'Found most often',
    },
    'show-details-label': 'show details',
    'catalog': 'Catalog',
    'most-often-added': 'Most often added',
    'open-category': 'Open category',
    'category-page-description': 'Here are all the products from this category',
    'minimum-order-value': 'Minimum order value',
    'maximum-order-weight': 'Maximum order weight',
    'maintenance-message': 'The website is under maintenance. Orders are on hold.',
    'tech-support-label': 'Technical support',
    'toast': {
      'in-process': 'In process',
      'updating-data': 'Data is being updated',
    },
  },
  'web-app-selector': {
    'search': 'Search...',
    'not-found': 'Nothing found',
  },
  'account': {
    'login-title': 'Sign in',
    'register-title': 'Create account',
    'my-account-title': 'My account',
    'email-label': 'Email address',
    'name-label': 'Name',
    'continue-with-google': 'Continue with Google',
    'divider-or': 'or',
    'submit-register': 'Create account',
    'switch-to-register': 'New here? Create an account',
    'switch-to-login': 'Already have an account? Sign in',
    'order-history-title': 'Your orders',
    'no-orders-yet': 'No orders yet',
    'loyalty-title': 'Your loyalty card',
    'loyalty-stamps-label': 'Stamps collected',
    'loyalty-reward-available': '20% off your next order (min. 20 €) is unlocked!',
    'loyalty-next-reward-note': 'Every 6 orders: 20% off your next order, minimum order value 20 €',
  },
  'common': {
    'time': {
      minute: '{n} minute | {n} minutes',
    },
    'hello': 'Hello',
    'select': 'Select',
    'optional': 'Optional',
    'sign-in': 'Sign in',
    'sign-out': 'Sign out',
    'login': 'Login',
    'password': 'Password',
    'return': 'Return',
    'actions': 'Actions',
    'open-page': 'Open page',
    'open-on-new-tab': 'Open on new tab',
    'yes': 'Yes',
    'no': 'No',
    'ok': 'OK',
    'more-information': 'More information',
    'home': 'Home',
    'to-home': 'To Home',
    'website': 'Website',
    'description': 'Description',
    'composition': 'Composition',
    'price': 'Price',
    'weight-or-volume': 'Weight / Volume',
    'measurement-unit': 'Measurement unit',
    'sku': 'SKU',
    'slug': 'Human-friendly URL',
    'photo': 'Photo',
    'icon': 'Icon',
    'key': 'Key',
    'value': 'Value',
    'required': 'Required',
    'footer': {
      'copyright': 'Copyright',
      'copyright-part-one': 'Created with',
      'copyright-part-two': 'on',
    },
    'abbreviation': {
      min: 'min',
      kg: 'kg',
      g: 'g',
      kcal: 'kcal',
      ml: 'ml',
      l: 'l',
      pcs: 'pcs',
      oz: 'oz',
      lb: 'lb',
    },
    'weight-unit': {
      g: 'Gram',
      kg: 'Kilogram',
      ml: 'Milliliter',
      l: 'Liter',
      pcs: 'Piece',
      oz: 'Ounce',
      lb: 'Pound',
    },
    'nutrition': {
      'value-title': 'Nutritional value per 100 gram',
      'calories': 'Calories',
      'kcal': 'Kcal',
      'protein': 'Protein',
      'fat': 'Fat',
      'carbohydrate': 'Carbs',
    },
    'day': {
      monday: 'Monday',
      tuesday: 'Tuesday',
      wednesday: 'Wednesday',
      thursday: 'Thursday',
      friday: 'Friday',
      saturday: 'Saturday',
      sunday: 'Sunday',
    },
    'time-from': 'Time from',
    'time-to': 'Time to',
    'payment-type': {
      cash: 'Cash',
      card: 'Card',
      custom: 'Custom',
    },
    'language': 'Language',
  },
  'error': {
    'title': 'Error',
    'default': 'Something went wrong',
    'invalid-credentials': 'Invalid credentials',
  },
} as const
